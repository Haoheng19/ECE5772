...
counter = 1; % current/latest index in dataStore
bump = 0;
state = "";
...
while toc < maxTime
    ...
    elseif state == "backward"
        ...
        % Check if has moved backward for 0.25m
        %if abs((abs(dataStore.truthPose(counter, 2) - lastX)) / abs(cos(lastTheta))) >= 0.25;
        %if pdist([ lastX, lastY; dataStore.truthPose(counter, 2), dataStore.truthPose(counter, 3)], 'euclidean') >= 0.25;
        if sqrt((lastX - dataStore.truthPose(counter, 2)).^2 + (lastY - dataStore.truthPose(counter, 3)).^2) >= 0.25;
            SetFwdVelAngVelCreate(CreatePort, 0, 0 );
            state = "rotate";
        end
        
    elseif state == "rotate"
        SetFwdVelAngVelCreate(CreatePort, 0, -0.3 );
        ...
        % Check if has rotated for 30deg
        if delta_angle >= 0.5236;
            SetFwdVelAngVelCreate(CreatePort, 0, 0 );
            state = "move";
        end
        
    else % state == "move"
        [cmdV,cmdW] = limitCmds(0.3, 0, 0.49, 0.13);
...
savefig('plot_trajectory_backup_euclidean.fig')